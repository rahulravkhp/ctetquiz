...
// प्रश्नों की कुल संख्या प्राप्त करें
$stmt = $conn->prepare("SELECT COUNT(*) AS total_questions FROM questions WHERE year IS NULL");
$stmt->execute();
$result = $stmt->get_result();
$total_available_questions = $result->fetch_assoc()['total_questions'];
$stmt->close();

// 150 या उपलब्ध प्रश्नों की संख्या तक रैंडम प्रश्न लोड करें
$limit = min(150, $total_available_questions);
$stmt = $conn->prepare("SELECT id, question_text, option1, option2, option3, option4, correct_option FROM questions WHERE year IS NULL ORDER BY RAND() LIMIT ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO results (student_id, score, total_questions, user_answers, created_at) VALUES (?, ?, ?, ?, NOW())");
    $stmt->bind_param("iiis", $_SESSION['user_id'], $score, $total_questions, $answers_json);
    $stmt->execute();
...
// पिछले प्रश्न पत्र लोड करें
$stmt = $conn->prepare("SELECT id, question_text, option1, option2, option3, option4, correct_option FROM questions WHERE year IS NOT NULL ORDER BY year DESC LIMIT 10");
$stmt->execute();
$previous_questions_result = $stmt->get_result();
$previous_questions = $previous_questions_result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
?>
...
<head>
    ...
    <title>टाइमर के साथ MCQ टेस्ट - CTET मॉक टेस्ट</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="styles.css">
</head>
<body>
    <div class="container mt-5">
        <h2 class="text-center">CTET मॉक टेस्ट (मोबाइल रेस्पॉन्सिव)</h2>
        <?php if ($score !== null) { ?>
            <div class="score">
            ...
            <div class="d-flex flex-wrap gap-2 justify-content-center mt-4">
                <a href="test_mobile.php" class="btn btn-primary">टेस्ट पुनः शुरू करें</a>
        ...
        <?php } else { ?>
            <div class="timer">
                <p>शेष समय: <span id="timer">30:00</span></p>
            </div>
                    ...
                    <div class="test-question <?php echo $index === 0 ? 'active' : ''; ?>" id="question-<?php echo $index; ?>">
                        <div class="question">
                            ...
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="q<?php echo $question['id']; ?>-1"><?php echo htmlspecialchars($question['option1']); ?></label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="answer[<?php echo $question['id']; ?>]" id="q<?php echo $question['id']; ?>-2" value="2">
                                <label class="form-check-label" for="q<?php echo $question['id']; ?>-2"><?php echo htmlspecialchars($question['option2']); ?></label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="answer[<?php echo $question['id']; ?>]" id="q<?php echo $question['id']; ?>-3" value="3">
                                <label class="form-check-label" for="q<?php echo $question['id']; ?>-3"><?php echo htmlspecialchars($question['option3']); ?></label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="answer[<?php echo $question['id']; ?>]" id="q<?php echo $question['id']; ?>-4" value="4">
                                <label class="form-check-label" for="q<?php echo $question['id']; ?>-4"><?php echo htmlspecialchars($question['option4']); ?></label>
                            </div>
                        </div>
                    </div>
                <?php } ?>
        ...
        <?php } ?>
        <div class="previous-questions mt-5">
            <h3>पिछले प्रश्न पत्र</h3>
            <?php foreach ($previous_questions as $question) { ?>
